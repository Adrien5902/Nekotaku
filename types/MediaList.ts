...
export function autoUpdateMediaListByProgress(media: Pick<Media, "episodes">, mediaList: Pick<MediaList, "progress" | "startedAt" | "completedAt" | "status">): Partial<MediaList> {
	...
	const isWatchingStatus = mediaList.status === MediaListStatus.Current || mediaList.status === MediaListStatus.Repeating;

	const status = hasWatchedAllEpisodes ? MediaListStatus.Completed : mediaList?.progress !== 0 && !isWatchingStatus ? mediaList.status === MediaListStatus.Completed ? MediaListStatus.Repeating : MediaListStatus.Current : mediaList.status;
